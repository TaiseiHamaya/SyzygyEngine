#include "StaticMeshDrawExecutorForward.h"
...
StaticMeshDrawExecutorForward::StaticMeshDrawExecutorForward(std::shared_ptr<const PolygonMesh> asset_, u32 maxInstance) {
	reinitialize(asset_, maxInstance);
}

void StaticMeshDrawExecutorForward::reinitialize(std::shared_ptr<const PolygonMesh> asset_, u32 maxInstance_) {
	asset = asset_;
...
void StaticMeshDrawExecutorForward::draw_command() const {
...
void StaticMeshDrawExecutorForward::write_to_buffer(Reference<const StaticMeshInstance> instance) {
